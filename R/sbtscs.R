...
sbtscs <- function (data, event, tvar, csunit, pad.ts = FALSE)
{
  tvar <- enquo(tvar)
  event <- enquo(event)
  csunit <- enquo(csunit)
  data <- dplyr::arrange_(data, csunit, tvar)
  sumevents <- data %>% group_by(!! csunit) %>% mutate(tot = sum(!!event))
  noevents <- sumevents %>% group_by(!! csunit) %>% filter(tot == 0) %>% mutate(spell = seq_along(!!tvar) -1)
  data <- sumevents %>% group_by(!! csunit) %>% filter(tot > 0) %>% as.data.frame()
  tvar <- quo_name(tvar)
